...
class Text {
  ...
  reset(x, y, recalcWidth) {
    let w = Math.min(this.cnt.width - x, this.width);
    let h = Math.min(this.cnt.height - y, this.height);
    ...
    this.cnt.displayGroup.add(this.text);
  }
  ...
  centerHorizontally() {
    this.text.cameraOffset.x =
      this.text.maxWidth / 2 - this.text.width / 2 + this.cnt.x;
    return this;
  }
   ...
   */
  get x() {
    return this.text.cameraOffset.x - this.cnt.x;
  }
   ...
   */
  set x(val) {
    this.text.cameraOffset.x = val + this.cnt.x;
  }
   ...
   */
  get y() {
    return this.text.cameraOffset.y - this.cnt.y;
  }
   ...
   */
  set x(val) {
    this.text.cameraOffset.y = val + this.cnt.y;
  }
